...
${crew.skipSmall
    ? `You may skip the plan only when the whole request is a single small change: one or two files, one thing to do, nothing to test separately. Then send it straight to the right helper. Anything with separate parts (several commands or features, code plus tests plus docs, more than one file that each need real work) gets a plan and gets split across levels, even if each part is easy.`
...
function writeCrewFiles(dir, crewIn) {
  ...
  for (const l of crew.levels) fs.writeFileSync(path.join(plug, 'agents', `${l.id}.md`), agentFile(l));
...
module.exports = { DEFAULT_CREW, cleanCrew, writeCrewFiles, PLUGIN };
